// app-v2.js - 智能增强版视频分析工具 (第二版本 - 多API支持)
...
function updateAPIStatus() {
    ...
    const currentProvider = ocrManager.getCurrentProvider();
    const providerStatus = ocrManager.getProviderStatus();
    
    // 更新当前提供商显示
    if (currentProviderSpan) {
        currentProviderSpan.textContent = getProviderDisplayName(currentProvider);
    }
    
    // 更新API状态指示器
    if (apiStatusIndicator) {
        const providerInfo = providerStatus[currentProvider];
        if (providerInfo && providerInfo.isHealthy) {
            apiStatusIndicator.className = 'api-status healthy';
            apiStatusIndicator.textContent = '✅ API正常';
        } else {
            apiStatusIndicator.className = 'api-status error';
            apiStatusIndicator.textContent = '❌ API异常';
        }
    }
}
...
function updateDebugInfo() {
    const providerInfo = ocrManager ? ocrManager.getCurrentProvider() : 'none';
    const providerStatus = ocrManager ? ocrManager.getProviderStatus() : {};
    ...
    debugInfo.innerHTML = `
        ...
        OCR提供商: ${getProviderDisplayName(providerInfo)}<br>
        API状态: ${Object.keys(providerStatus).length > 0 ? '已连接' : '未连接'}
    `;
}
...
function updateAnalysisStats(stats) {
    ...
    analysisStats.innerHTML = `
        ...
        <strong>OCR状态:</strong><br>
        当前提供商: ${getProviderDisplayName(ocrManager?.getCurrentProvider() || 'none')}<br>
        提供商状态: ${getProviderHealthStatus()}
    `;
}

// --- 获取提供商健康状态 ---
function getProviderHealthStatus() {
    if (!ocrManager) return '未初始化';
    
    const status = ocrManager.getProviderStatus();
    const current = ocrManager.getCurrentProvider();
    
    if (status[current]) {
        return status[current].isHealthy ? '✅ 健康' : '❌ 异常';
    }
...
function updateOcrPreview() {
    ...
    try {
        ...
        const providerName = ocrManager?.getCurrentProvider() || 'none';
        ...
        ocrPreviewInfo.innerHTML = `
            ...
            OCR提供商: ${getProviderDisplayName(providerName)}
        `;
...
function updateOcrProcessedPreview() {
    ...
    try {
        ...
        const providerName = ocrManager.getCurrentProvider();
        
        ocrProcessedPreviewInfo.innerHTML = `
            预处理: 基础二值化<br>
            OCR引擎: ${getProviderDisplayName(providerName)}<br>
            状态: ${getProviderHealthStatus()}
        `;
...
function showStrategyPreviews(strategyCanvases) {
    ...
    if (!strategyCanvases || strategyCanvases.length === 0) {
        strategyPreviews.innerHTML = '<div style="text-align: center; color: #666;">无策略预览</div>';
        return;
    }
...
async function initializeOCR() {
    ...
    try {
        console.log("开始初始化统一OCR管理器");
        
        // 确保配置和OCR管理器已加载
        if (!window.ocrManager) {
            throw new Error('OCR管理器未加载');
        }
        
        ocrManager = window.ocrManager;
        
        updateProgress(25, '初始化OCR提供商...');
        await ocrManager.initialize();
        
        updateProgress(75, '更新API状态...');
        updateAPIStatus();
        
        updateProgress(-1, 'OCR 服务已就绪。请上传视频。');
        console.log("统一OCR管理器初始化完成");
        ...
        if (error.message.includes('API密钥')) {
            statusMessage.innerHTML = 'OCR服务初始化失败：请点击"配置"按钮配置API密钥。';
        } else {
            statusMessage.innerHTML = `OCR服务初始化失败：${error.message}`;
        }
    }
}
...
testOcrBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        // 显示策略预览
        const strategyCanvases = ocrManager.getStrategyCanvases();
...
async function performSingleOCR(frameTime, occurrenceIndex, isTest = false) {
    try {
        ...
        // 使用统一OCR管理器
        const ocrResult = await ocrManager.processImageWithMultipleStrategies(tempCanvas, tempCtx);
        
        console.log(`OCR结果 (时间${frameTime.toFixed(2)}s):`, ocrResult);
        ...
        return {
            ...
            strategy: ocrResult.strategyName,
            provider: ocrResult.provider,
            hasDecimalPoint: ocrResult.hasDecimalPoint,
...
function displayOCRTestDetails(result) {
    ...
    ocrTestResult.innerHTML = `
        ...
        - OCR提供商: ${getProviderDisplayName(result.provider)}<br>
        ...
        analysisResults.forEach(result => {
            tableHTML += `
                <tr>
                    ...
                    <td>${getProviderDisplayName(result.provider) || 'N/A'}</td>
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            ...
                            return `数字: ${context.parsed.y} (时间: ${pointData.frameTime.toFixed(2)}s, 提供商: ${getProviderDisplayName(pointData.provider)})`;
                        }
                    }
                }
            }
