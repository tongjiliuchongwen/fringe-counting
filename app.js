...
let canvasInternalWidth = 0;  // 等于 video.videoWidth
let canvasInternalHeight = 0; // 等于 video.videoHeight
...
let scaleX = 1;               // 用于将鼠标坐标映射到视频内部像素的缩放因子（宽度方向）
let scaleY = 1;               // 用于将鼠标坐标映射到视频内部像素的缩放因子（高度方向]
...
videoPlayer.addEventListener('loadedmetadata', () => {
    ...
    // 在这里获取视频在页面中的实际显示大小，用于计算缩放比
    const rect = videoPlayer.getBoundingClientRect();
    // 如果你想让 drawingCanvas 与视频在页面上呈现相同大小，就这么设置:
    drawingCanvas.width = rect.width;
    drawingCanvas.height = rect.height;

    // 计算内部像素与显示区域的缩放因子
    scaleX = canvasInternalWidth / rect.width;
    scaleY = canvasInternalHeight / rect.height;

    console.log(`视频实际分辨率: ${canvasInternalWidth}×${canvasInternalHeight}`);
    console.log(`视频在页面显示大小: ${rect.width}×${rect.height}`);
    console.log(`坐标缩放因子: scaleX=${scaleX}, scaleY=${scaleY}`);
...
drawingCanvas.addEventListener('mousedown', (e) => {
    ...
    // 拿到与视频内部像素对应的坐标
    const xInCanvas  = (e.clientX - rect.left) * scaleX;
    const yInCanvas  = (e.clientY - rect.top ) * scaleY;
...
async function performOCR(frameTimeAtMaxBrightness, occurrenceIdx) {
    ...
    const WHITE_THRESHOLD = 200; // 可以根据画面尝试调 180~220 左右
    ...
    // 最后，把二值化后的图片交给 Tesseract
    try {
        const psmValue = '7'; // 可以尝试 6/8/10 等模式
